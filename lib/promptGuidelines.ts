...
Most-abused shapes (these slip through most on punchy hooks, titles, and subject lines — if a line lands on one, recast it as a direct claim or a concrete image before returning): "It's not X, it's Y" / "not X, but Y"; "You don't need another X"; "You don't have an X problem, you have a Y problem" / "That's an X problem"; "You don't have an X problem" as a standalone opener (banned even without the "you have a Y problem" completion); "X isn't broken" / "Your X isn't broken". Recast example: "It's not a marketing problem, it's the friend zone" becomes "Your warmest followers ask you for advice, then pay someone else."
