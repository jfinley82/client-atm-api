...
import { COACH_SHELL_URL, appUrl } from '../../lib/appUrls'

// COACH_SHELL_URL lives in lib/appUrls with the app's own base, so the two
// cannot drift the way four copies of APP_URL did. This link got the wrong host
// twice — first by inheriting APP_URL when that pointed at the pre-cutover app,
// then by being handed its own hardcoded guess — and both times it looked
// correct until a lead would have clicked it.
      ...
      if (outcome.action === 'ai_assistant' && lead?.id) {
        try {
          ...
          if (entitled && aiCoach?.confirmed === true) {
            ...
            coachUrl = `${appUrl(COACH_SHELL_URL, 'coach')}?t=${encodeURIComponent(token)}`
          }
        } catch (err) {
